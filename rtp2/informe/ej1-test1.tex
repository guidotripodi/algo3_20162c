\indent Veremos que es lo que sucede al ir variando los parámetros de entrada para cada familia, de manera tal que, a medida que F y C crecen linealmente, podamos observar que sucede con cada familia de casos (es decir, las familias se mantienen, solo varia el tamaño de la entrada en todos los casos).\\
...
Dado que las familias de casos vienen dadas por la cantidad y disposición de las paredes en el mapa y el valor del $P_{max}$ para cada una de las familias tiene sentido si se asigna un valor determinado, el caso de análisis donde se varía el tamaño de entrada y la cantidad y disposición de paredes, dejando fijo $P_{max}$, carece de sentido analítico ya que ,  

%Para poder evidenciar la complejidad del algoritmo sobre las mediciones tomadas, al depender la misma de F*C cantidad de elementos de la entrada, y P un parámetro que solo influye siempre y cuando $P<F*C$, se necesitaria poder variar F*C con P fijo y vice versa. No obstante esto solo tiene sentido al variar el parametro P con F*C fijo, ya que en el caso contrario, 
