...
\While{sePudoDeshacerEleccion}{  
	\If{lesGaneATodos(MaestroPokemon) $\wedge$ tiempo(MaestroPokemon) < minimo}{
	 \hfill  guarda: O(1)\\
			...
			sePudoDeshacerEleccion asginar deshacerEleccion(MaestroPokemon) \hfill O(1)\\
		}
		}
...
\Fn{aplicarEleccion(eleccion, maestroPokemon)}{
	...
	\hfill  guarda: O(1)\\
		...
		para maestroPokemon.cantidadPociones  asignar pociones(maestroPokemon) - pocionesNecesarias(eleccion) \hfill O(1) \\
	}\Else{
		para maestroPokemon.cantidadPociones  asignar pociones(maestroPokemon) + 3\hfill O(1) \\
