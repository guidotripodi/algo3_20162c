...
Con respecto a la diferencia entre la soluciones que se obtienen en relacion a las optimas elebaramos las siguientes comparaciones:\\\\\\
...
Al generarse un camino alternado para el goloso entre gimnasio y pokeparada, se le dan a todos los gimnasios la dificultad igual a la cantidad de pociones que aporta cada pokeparada y se crean la misma cantidad de gimnasios como de pokeparadas. En cuando a la distribuciòn de los mismos, se busco una forma de estrella, en la cual cada pokeparada está alineada con 2 gimnasios y una segunda pokeparada. Dadas estas restricciones se observa lo siguiente:
...
Finalmente, como $k = j + 1 \wedge k+j=n+m-1 \leftrightarrow j = n - 1 $, se resolverán distintos caminos siempre y cuando $a +c \ge b + d$. Con lo cual, para generr correctamente las instancias para esta famila, se considero dicha desigualdad.
...
Para los n hasta 8 se usaron distintos radios que para los casos màs grandes. En sintesis se puede ver que el tiempo para los tamaños del 10 al 14 para el algoritmo exacto, se mantuvo la distancia recorrida: esto se debe al caracter del camino. El camino óptimo resulta de recorrer primero 1 circunferencia, luego dirigirse de forma recta a la circunferencia interior y recorrerla. Al no haber variabilidad en los radios, el tiempo insumido es aproximadamente el mismo. No así para el algoritmo goloso, ya que un incremento en la cantidad de elemento, conyeva un incremento de las idas y vueltas a realizar; aumentando notoriamente la distancia del recorrido.
...
Se evidencia el mejor desempeño del algoritmo sobre la familia 6, provocando un error medio mucho menor que la familia 4 (alrrededor de 110\% mejor) y con una dispercion del mismo mucho más baja. 

De forma separada, debido a los rangos que se manejan, se decidió analizar la distribución de la Familia 7. Vale destacar que el error de la misma es muy inferior al de las 2 anteriores y posee la menor varianza de todas. Esto se debe al caracter controlado de la misma, y su caracteristica de posicionamiento: 
\vspace*{0.3cm} \vspace*{0.3cm}
  \begin{center}
