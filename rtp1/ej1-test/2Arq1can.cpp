...
int main(int argc, char* argv[])
{
	...
	for(int j = 0; j < CANT_MAX; j++){
			tiempos_arqueologos[j] = j + 1;//no cuento el cero
			tiempos_canibales[j] = j + 1;
			cant_canibales = j + 1;
			cant_arqueologos = (j + 1)*2;
			for (int x = j; x < cant_arqueologos; ++x){
				tiempos_arqueologos[x] = x + 1;
			}
			if (cant_canibales + cant_arqueologos <= 7)	{
